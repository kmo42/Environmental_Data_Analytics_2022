...
>questions related to trends, and look exactly at trend components 
...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #last quotes add separator space 
  mutate( Date = my(paste0(Month,"-",Year))) %>%  #paste0 pastes all the elements together
  select(Date,Temperature_C)  


######my function reads in as.date function; take stats column and split in 2 with separate function to get month on own, 
#####text to column function in excel; manually place errors, 
###sep stats and add space to get 
###then mutate month and 
###null = column not going to use to represent  space and average
...
####inner join function makes the data and removes gaps with creating vector with 
###some datasets omitted rows for dates with NA
...
#Exercise 5: Check if there is any missing data for both temperature and rainfall series. ###plot and look for NAs 

summary(BRA_complete$Temperature_C)

summary(BRA_complete$Rainfall)
##sum function shows if data is equally spaced, number of rows, use summary and verify no nAs



#Exercise 6: Plot both series over time on separate plots. Add a trend line using geom_smooth(). Can you say something about the trend just by visualizing the data? ###add trendline
...
###make sure pointing out column to ts function, if do more than one column at a time, have to keep track of which vector is which

fmonth <- month(first(BRA_complete$Date))
fyear <- year(first(BRA_complete$Date))

BRA_R_ts <- ts(BRA_complete$Rainfall, start = c(fmonth,fmonth), frequency = 12)
BRA_T_ts <- ts(BRA_complete$Temperature_C, start = c(fmonth,fmonth), frequency = 12) #frequency tells the seasonal part of the dataset, repeat every 12 observations to show new year, monthly data = 12, daily data= 365
...
###decompose into three diff and all added together equal the original
...
# components of each series and turn them into data frames. (in lesson)
...
# Exercise 10: Visualize how the trend maps onto the data for both series (add the trend component and seasonal component)
