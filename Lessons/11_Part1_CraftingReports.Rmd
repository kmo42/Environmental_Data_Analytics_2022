...
author: Environmental Data Analytics | John Fay & Luana Lima | Developed by Kateri Salk | Student Katie Owens9
date: "Spring 2022"
output:
...
**R Markdown** is an *excellent* tool for creating documents with integrated code including other features: 
...
-------------- | -----------
cb_2018_us_county  |  Stream gage height data in Nebraska
NTL-LTER_Lake  |  Data from lakes in Wisconsin. 
...
NTL.nutr.data_A11 <- read.csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv", stringsAsFactors = TRUE)
head(NTL.nutr.data_A11)

NTL.nutr.data_A11$sampledate <- as.Date(NTL.nutr.data_A11$sampledate, format = "%m/%d/%y")
head(NTL.nutr.data_A11)
...
Create an R chunk below to create a processed dataset and do the following operations:
...
```{r make pr.dataset}
#first try:
#  summary(NTL.nutr.data.0depth) #check to see only have depth=0

#NTL.noNA <- NTL.nutr.data.0depth %>% drop_na()
#summary(NTL.noNA) #confirmed remove NAs

as.numeric(NTL.nutr.data_A11$depth)
#second try
NTL.lakes <- NTL.nutr.data_A11 %>%
...
```{r include=FALSE}
#summary stats
NTL.summary_data.N <- NTL.lakes %>%
  group_by(lakename) %>%
  summarise(meanN = mean(tn_ug), 
            sdN = sd(tn_ug), 
            minN = min(tn_ug),
            maxN = max(tn_ug))
head(NTL.summary_data.N)

NTL.summary_data.P <- NTL.lakes %>%
  group_by(lakename) %>%
  summarise(meanP = mean(tp_ug), 
            sdP = sd(tp_ug),
            minP = min(tp_ug),
            maxP = max(tp_ug))
head(NTL.summary_data.P)
...
kableExtra::kable(NTL.summary_data.P, format = 'simple', caption = "Total Phosphorus Concentrations (ug)")
kableExtra::kable(NTL.summary_data.N, format = 'simple', caption = "Total Nitrogen Concentrations (ug)")
```
...
```{r plot1, echo=FALSE, fig.cap = "Total Phosphorus Over Time", fig.align = 'left'}
#plot of P

ggplot(NTL.lakes) +
  geom_point(aes(x = sampledate, y = tp_ug, color = lakename, shape = lakename)) +
  labs(y="Total Phosphorus (ug)", color = "Legend")

```

```{r plot2, echo=FALSE, fig.cap = "Total Nitrogen Over Time", fig.align = 'left'}
#plot of P

ggplot(NTL.lakes) +
  geom_point(aes(x = sampledate, y = tn_ug, color = lakename, shape = lakename)) +
  labs(y="Total Nitrogen (ug)", color = "Legend")
...
This coding challenge was interesting and resulted in finding a trend of a wider range and increasing slope of nitrogen and phosphorus concentrations over time in the lakes of Wisconsin. In 1996 there seemed to be a spike in nitrogen levels, excluding Paul Lake, that lowered to relatively normal levels rather quickly afterwards. 



To further explore this dataset, it would be fun to analyze the trends of phosphorus and nitrogen concentrations at varying depths and then see how the vertical stratification through the water column matches up with measurements taken at the surface. To answer these curiosities, one could plot each nutrient over a range of depths and concentrations with incremental years to show the progression through time. Also, conducting a time series analysis would help with identifying factors of high and low influence. 
