...
>A06:#5 (Mutate function to make a new column, and then filter for month code, 7 for july), 9728 observations with no NAS
...
>comparing mean on two levels
>running one way Anova, with more than one level with two-sample t test
...
#Exercise 2:
#year in x axis 
#make interger year into as.factor
#add transparency with alpha function
#scale maual fill"
#labs (x = ozone and y  = 
#Overdlay, when choose dark colors they become light
#why chose fill which is better, if you use color, only applies to line,
#if shoooser ffill to change color under the curve 
...
#boxplot with x = year with as.factor, y = ozone
#add letters with stat_summary (geom = "text", fun.y=max) for function of where to place the text (which max equals maximum; 
#vjust means vertianckl boxlots 
#labs(x="", y= ")
#adjust limits to be able to see label a with ylim




#Exercise with violinn
#ggplot(
#geom_violin (draw quantiles = 0.5 for horizontal line)
#stat_summary)
...
```
>Ends in low p-value which is good; 
>low rsq bc points are all over the place bc missing some variables
