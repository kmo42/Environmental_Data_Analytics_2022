...
> Add notes here: for looking at the changes to lake ph and chemistry related to environmental effects over time
...
#rows: slice= based on location 
#columns: mutate=useful to create a new columns based on existing values in a columns; 
#relocate= move columns into groups
```
...
`|`
```{r}
`==` #compare value to a specific value, if/for/if statements
`!=` #check if values are equal
`<` 
`<=`
`>`
`>=`
`&` #checking multiple conditions if want both to true
`|` #checking mult. conds if one true and other false

```

```{r}
class(NTL.phys.data$lakeid)#can only compare things with sim classes, char with char; num with num
class(NTL.phys.data$depth) #with num depth, can use >< notation

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]
    #filtering the rows that have 0 in column depth

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
    #filter dataset, column with 0s
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)
    #filter original dataset for depths with less than 0.25 
...
summary(NTL.phys.data$daynum)#151 not included range from 152-304 with 305 not included bc is nov 1st
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) # & and , yield same results
