...
>Ben in class has taken time series analysis
...
* **Augmented Dickey Fuller**: no seasonality, non-parametric, missing data not allowed. Function: `adf.test()` (package: tseries) (for stochastic trends)

>stochastic trend vs deterministic trend, Stochastic: trend that changes over time; deterministic: constant over time
...
###P-value of less than 0.05 =reject the null hypothesis (no trend) and have a trend, support for the alternative hypothesis saying there is a trend present
...
#KO NOTES
###Tau for each season of the year shown by S, S is shows stronger tendency of decrease for some seasons, 
###p value higher than 0.05 = stationary, tendency of decrease is not well pronounced with low negative values,  stronger tendency of decrease with higher negative S values,  
###With p-values higher than 0.05, those seasons have a stationary trend, but overall decreasing trend on data set with most of the p-values being less than 0.05 showing a change and not stationary trend (decreasing, this time)


#again from class site version of instructions
#decompose
wind_data_decomp <- stl(wind_data_ts,s.window = "periodic")
plot(wind_data_decomp)
# Run SMK test
...
#linear regression in blue can see the negative slope, showing a decreasing trend of average wind speed over time as we were able to spot with the decomp and as we are able to conclude with the seasonal mann kendall test.
...
>If at any moment, need some forecasts, the ARMA are the simplest version to find functions to apply to already composed datasets
