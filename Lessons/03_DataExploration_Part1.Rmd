...
1. Click# We will use the packages dplyr and ggplot2 regularly. 
install.packages("dplyr") 
install.packages("ggplot2")
install.packages("tidyverse")
# comment out install commands,  the box next to the package name in the Packages tab. 
...
```{r packages}
#use only when needed and re-comment
...
<<<<<<< HEAD
# Relative file path (friendly for users regardless of machine)#KO: better for long-term use
...
# What happens if we don't assign a name to our imported dataset? #need to import into environment with this line
#USGS.flow.data <- read.csv("../Data/Raw/USGS_Site02085000_Flow_Raw.csv") 
...
> ANSWER: It is in the metadata file in the data file for the class. It is good placement of the metadata being separated from the other types of files like output and lessons.
...
str(USGS.flow.data) #shows head of df
dim(USGS.flow.data) #shows #of rows and columns in dataset 
length(USGS.flow.data) #tells number of columns in data

head(USGS.flow.data) #1st 6 rows
head(USGS.flow.data, 10) #show first 10 rows
tail(USGS.flow.data, 5) #shows last 5 rows of data
USGS.flow.data[30000:30005, c(3, 8, 14)] #asking for 5 rows, with only these columns 3,8,14

class(USGS.flow.data$datetime)#to show what type of data is in the column
class(USGS.flow.data$discharge.mean) #numeric type
class(USGS.flow.data$gage.height.mean) #numeric type

summary(USGS.flow.data)   #gives the sum stats for every column #could point to column only with $

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: Turned into NA
...
> Answer: Good to remove NAs in old data sets about things like water heights with a ton of missing values. Important to retain or transform NAs for datasets regarding time series analysis research questions.
...
%A  unabbreviated weekday #whole word 
%b  abbreviated month
...
```{r fixformartDate}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") #turns datetime into character format to change, with format cunction

#
create.early.dates <- (function(d) { #creating function
       paste0(ifelse(d > 191226,"19","20"),d)
       })
#
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)#adds 19 or 20 prefix

#
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") #puts back in date format with as.date fcn
...
#saving data into csv and USGS.flow.data is the desired name of data file, fcn is write.csv, and need to specidfy folder to put data file, key in processed data!
=======
write.csv(USGS.flow.data, file = "./Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE)
>>>>>>> b3c7106116c07efb5453f2e1b6ba3c74dfab9fdd
