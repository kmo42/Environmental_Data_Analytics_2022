...
>8digit hucs, 277, 2770, 27701, big to small, the more digit you add the more you narrow, 
...
ggplot(data=huc8_sf) +
  ...
  scale_fill_continuous(style = viridis)
...
#Check structure
str(wind_raw)
#make new column, and substring command to do a new function
substr(wind_raw$HUC_12, 1, 8) #start at first charcter and go 8 charactoers
...
```
>>For joining, can use merge with by.x, by.y with HUC_8 or left_join
>quick plot using the sumwindenergy with new attribute
>for distance measurements, want projected coordinate system (big numbers for units, look in data), vs geographic
>given the command for one of these, 
>First put in UTM projection system by looking up UTMIO?
>first isolate, and select Durham, Chatham, etc
>give me the ear

>first make new data set with 4 counties, then subset values, 
>for filtering  st package isn't  , returnes huc_8_sf_htm
>can also do in matrice form
>google r sf cheat sheet for descriptions, some apply to columns, some apply to items
>st_intersection: one layer will be cookie dough, cutting cookie, first argument is cutter, second is the dough , can grab the data for free instead of ArcGIS, on the way. 
